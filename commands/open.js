...
module.exports = {
  ...
  run: async (client, interaction, config) => {
    ...
    const channel = guild.channels.cache.find(
      (x) => x.name === user.id && x.parentId === category.id
    );
    ...
    if (db.has(user.id)) {
      await interaction.reply({
        embeds: [
          new EmbedBuilder()
            .setDescription(
              `That user is banned from using the modmail with the reason below:\n> ${db.get(
                message.author.id
              )}`
            )
            .setColor("Red"),
    ...
    const mailchannel = await guild.channels
      .create({
        name: user.id,
        ...
        } to contact with ${user.tag} since ${new Date().toLocaleString()}.`,
      })
      .catch(() => {});
    ...
    await user.send({
      embeds: [
        new EmbedBuilder()
          .setTitle("New mail")
          .setDescription(
            "A staff member wants to contact with you, you can start to communicate by sending messages here."
          )
          .setColor("Yellow"),
      ],
      components: [
