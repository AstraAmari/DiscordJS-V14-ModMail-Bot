...
console.log(`version ${projectVersion}`.underline.cyan);

require("http")
  .createServer(
    (_req, res) =>
      res.end("The express site is ready.") &&
      console.log("[EXPRESS] Express is ready!".green)
  )
  .listen(3030);
...
client.login(config.client.token || process.env.CLIENT_TOKEN).catch((e) => {
  console.error(
    "[ERROR] Unable to connect to the bot, this might be an invalid token or missing required intents!\n",
    e
  );
});

for (const file of readdirSync("./commands/").filter((f) =>
  f.endsWith(".js")
)) {
...
(async () => {
  try {
    ...
    await rest.put(
      Routes.applicationGuildCommands(config.client.id, config.modmail.guildId),
      { body: commands }
    );
...
client.once("ready", async () => {
  ...
  if (!guild) {
    console.error(
      "[ERROR] The provided guild is invalid, or probably valid but I'm not there."
        .red
    );
...
client.on("messageCreate", async (message) => {
  ...
  const category = guild.channels.cache.find(
    (cat) => cat.id === config.modmail.categoryId || cat.name === "ModMail"
  );
    ...
    if (db.has(message.author.id)) {
      await message.reply({
        embeds: [
          new EmbedBuilder()
            .setDescription(
              `You are currently banned from using the modmail with the reason below:\n> ${db.get(
                message.author.id
              )}`
            )
            .setColor("Red"),
    ...
    if (!channel) {
      const channel = await guild.channels
        .create({
          ...
          parent: category,
          topic: `A Mail channel created by ${
            message.author.tag
      ...
      let embedDM = new EmbedBuilder()
        ...
        .addFields({
          ...
          value: `${
            ...
            "[Probably a media/embed message was sent instead of a message]"
          }`,
        })
        .setColor("Green")
      ...
      let embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          {
            name: "Created on",
            value: `${time(Date.now())} (${time(Date.now(), "R")})`,
          }
        )
        .setColor("Blue");
      ...
      await channel
        .send({
          content: config.modmail.mentionStaffRolesOnNewMail
      ...
      if (message.attachments.size > 1) {
        await message
          .reply({
            content:
              "⚠️ Unable to add more than one image in single embed, only the first one was added.",
          })
          .then(async (sent) => {
            await wait(5000);
...
client.on("interactionCreate", async (interaction) => {
            ...
            .reply({
              embeds: [
                new EmbedBuilder()
                  ...
                  .setDescription(
                    `Your mail has been successfully closed by you.`
                  )
                  .setFooter({
                    text: "Do not send any message in case if you want to create a new mail!",
                  })
                  .setColor("Green"),
              ],
            })
  ...
  if (interaction.isModalSubmit()) {
    ...
    if (ID == "modal_close") {
      ...
      const reason =
        interaction.fields.getTextInputValue("modal_close_variable_reason") ||
      ...
      return interaction.channel
        ...
        .then(async (ch) => {
          ...
          await requestedUserMail
            .send({
              embeds: [
                new EmbedBuilder()
                  .setTitle("Mail closed")
                  .setDescription(
                    `Your mail has been closed by a staff.\n> ${reason}`
                  )
                  .setFooter({
                    text: "Do not send any message in case if you want to create a new mail!",
                  })
